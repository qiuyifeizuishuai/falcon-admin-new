<template>
  <section class="base-list custom-input">
    ...
    <el-row class="center" v-loading="listLoading" :element-loading-text="loadText">
      <slot name="list"></slot>
    </el-row>

    <el-row class="bottom" type="flex" v-show="isPaging">
      <div class="oper">
        <slot name="oper"></slot>
      </div>
      <el-pagination
        class="page"
        @size-change="size=>pageInfo.size=size"
  ...
  methods: {
    loadData() {
      return new Promise((resolve, reject) => {
        ...
        /** 拼接分页条件 */
        params.start =
          this.pageInfo.pageStart >= 1 ? this.pageInfo.pageStart : 1;
        params.perPage = this.isPaging ? this.pageInfo.size : 99999999; // 判断是否对列表分页
        if (this.condition.perPage) params.perPage = this.condition.perPage;
        ...
        this.pageSearch(params).then(reponse => {
            ...
            if (pageData.records) pageData = pageData.records;
            if (!pageData) {
              pageData = [];
            }
