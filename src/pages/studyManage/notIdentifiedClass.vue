    ...
    >
      ...
      >
        ...
        <el-button
          ...
          type="success"
          icon="el-icon-plus"
          @click="$router.push({name:'addCurriculum',query:{isAdded:'use',type:'generalExamination'}})"
        >新增课程</el-button>
          ...
          <el-table-column label="课件" align="center">
              <template slot-scope="scope">
                  ...
                  >
                      ...
                      <el-button size="small" @click="upload=scope.row" v-else>上传课件</el-button>
                  </el-upload>
              </template>
          </el-table-column>
        <el-table-column prop="created_at" align="center" label="操作" width="180">
          <template slot-scope="scope">
            <div class="box">
              <span
                class="span"
                @click="$router.push({name:'addCurriculum',query:{uuid:scope.row.uuid,isAdded:'see',type:'generalExamination'}})"
              >编辑/查看</span>
              <em class="emClass"></em>
              <span
                class="span"
  ...
  methods: {
    ...
    loadData() {
      ...
      beforeAvatarUploadCourseware(file) {
          ...
          // if(file.type=='application/pdf'||file.type=='application/wps-writer'||)
          // const isJPG = file.type === 'application/pdf';
          // if (!isJPG) {
          //     this.$message.error('上传文件只能是 PDF/Word/PPT 格式!');
          // }
    ...
    created() {
        this.$emit('getMeteData', this.$Bre.breadCrumbs([0, 3, 35]));
    },
    mounted() {
